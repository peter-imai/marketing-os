...
- **Deep-path routing.** After scaffold, offer: "Want to do full positioning now?" → work through positioning with the agent (Dunford-style). "Want market research?" → routes to Crawford prompts. Currently just suggests these as next steps.
...
- **Import from external system.** If the operator has existing context (Notion, Google Docs, Cursor), offer to import and triage into the canonical doc structure. Related: `blueprints/client-context-architecture.md`.
